...
def search_by_author(author):
    ...
    for i in range(l):
        print(all_book[i][1])
        temp.append({'book_name':all_book[i][1],'author': all_book[i][2],'style':all_book[i][3],'count':all_book[i][4],'price':all_book[i][5],'img':all_book[i][6],'emoji':all_book[i][7],'page_count':all_book[i][8]})
    print(temp)
    return temp
    ...
    for i in range(l):
        ...
        if emoji in temp_emoji:
            ...
            temp.append({'book_name': book[1], 'author': book[2], 'style': book[3],
                     ...
                     'emoji': book[7], 'page_count': book[8]})
    print(temp)
    return temp
...
def search_by_style(style):
    ...
    for i in range(l):
        temp.append({'book_name': all_book[i][1], 'author': all_book[i][2], 'style': all_book[i][3],
                     ...
                     'emoji': all_book[i][7], 'page_count': all_book[i][8]})
    print(temp)
    return temp
